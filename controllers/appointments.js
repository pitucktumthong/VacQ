...
exports.getAppointments = async(req, res, next)=>{
    ...
    if (req.user.role !== 'admin'){
        ...
        }else{// If you are admin , you can see all appointment
...
exports.addAppointment = async(req, res, next)=>{
    try{
        ...
        //If the user is not an admin, they can only operate 3 appointments.
        if(existedAppointments.length>=3 && req.user.role !="admin"){
            return res.status(400).json({success:false,message:"The user wiht ID "+req.user.id+"has already made 3 appointments"});
        }

        const appointment = await Appointment.create(req.body);
        return res.status(202).json({success:true,data:appointment});
...
exports.updateAppointment = async(req, res, next)=>{
    try{
        ...
        //Make sure user is the appointment ownership
...
//route DELTE api/v1/appointments/:id
//@access Private
